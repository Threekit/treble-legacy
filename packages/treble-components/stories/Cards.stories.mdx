...
import Cards, { Cards as CardsComponent } from '../src/Cards';

<Meta title="Components/Cards" />

export const CardsComponentTemplate = args => <CardsComponent {...args} />;
export const CardsTemplate = args => <Cards {...args} />;
...
<Canvas>
  ...
  >
    {CardsComponentTemplate.bind({})}
  </Story>
</Canvas>

<Canvas>
  <Story
    name="Cards Plugged In"
...
**Cards** are great for displaying option sets where each option only
needs to display a title.

## Overview

The `<Cards />` component will render out 2 or more columns of Tile components, where each Tile
represents an option in the options array. The Cards form a neat grid.
